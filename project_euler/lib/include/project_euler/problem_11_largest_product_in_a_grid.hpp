...
namespace project_euler
{
	...
	public:
		...
		static constexpr const char* problem_string =
		...
		static constexpr const char* name = "Largest product in a grid";

		/**
		 * @brief
		 * Uses brute force to go through the numbers and checking their
		 * adjacent digits. The time can be reduced a little by checking
		 * only four direction for every number since the 'backwards'
		 * directions will be checked by earlier iterations
		 */
		static std::size_t Naive(const std::string& grid, std::size_t adjacent_digits);
